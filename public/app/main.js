...
const I18N = {
  zh: {
    ...
    lan_title_on: '局域网访问已开启 — 手机/其他电脑可访问',
    lan_title_off: '局域网访问已关闭 — 只有本机能访问',
    // v0.5.aw: 工作区/会话相关
    workspace_unset: '（未设置）',
  ...
  en: {
    ...
    lan_title_on: 'LAN access enabled — phone/other PCs can access',
    lan_title_off: 'LAN access disabled — only this PC can access',
    // v0.5.aw: 工作区/会话相关
...
let currentLang = localStorage.getItem('webui-lang') || 'en'
...
const FIVE_HOUR_BOUNDARIES = [5, 10, 15, 20, 24]

function nextFiveHourReset(now = new Date()) {
  // 返回下一次 5h 窗口的本地时间
...
function formatResetTime(target) {

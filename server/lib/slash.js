...
import { loadSessions, saveSessions, persistCurrentChat, resetContext } from './sessions.js'
import { ensureMcodeCommands, getCachedMcodeCommands } from './acp-client.js'
...
export async function handleCmdCommand(cmd, cs, cid) {
  ...
  if (cmd === '/stop') {
    const { getActiveChild, clearActiveChild } = await import('./state-bus.js')

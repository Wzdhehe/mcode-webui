...
export function getMcodeBetterSqlite3({ MCODE_RUNTIME_DB: _ignored } = {}) {
  ...
  try {
    ...
    _McodeBetterSqlite3 = _webuiRequire(join(__cfg, 'node_modules', '@minimax-ai', 'code', 'node_modules', 'better-sqlite3'))
...
export function deleteMcodeSessionFromDb(sid, { MCODE_RUNTIME_DB } = {}) {
  ...
  if (!MCODE_RUNTIME_DB || !existsSync(MCODE_RUNTIME_DB)) return { ok: false, reason: 'mcode_db_not_found' }
  let db
  try {

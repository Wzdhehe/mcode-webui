...
export function handleWorkspaceChange(cs, cid, payload) {
  ...
  if (action === 'useTui') {
    target = detectTuiCwd()
    if (!target) return { ok: false, error: 'mcode TUI 还没启动过，没有 cwd 记录' }
  } else if (action === 'reset') {
    target = DEFAULT_WORKSPACE
  } else if (action === 'detect') {
    const tui = detectTuiCwd()
    return { ok: true, tuiCwd: tui, defaultWorkspace: DEFAULT_WORKSPACE, current: cs.workspace.dir, detectOnly: true }
  } else {
    target = payload.dir
  }
  if (!target || typeof target !== 'string') return { ok: false, error: 'dir 不能为空' }
  // 校验目录存在
  ...
  pushStateFor(cid)
  return { ok: true, workspace: cs.workspace, tuiCwd: detectTuiCwd(), defaultWorkspace: DEFAULT_WORKSPACE }
}
...
export function browseWorkspace(rawPath) {
  ...
  if (target) {
    ...
    children.push(...dirs)
    return { ok: true, dir: target, parent, children, skipped, total: dirs.length }
  } else {

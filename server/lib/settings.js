...
export function getLanBroadcast() { return lanBroadcastEnabled }
...
const LAN_REJECT_HTML = (remoteIp) => `<!DOCTYPE html><html><head><meta charset="utf-8"><title>webui — 局域网访问已关闭</title>
<style>
...
export function rejectLan(res, pathname, remoteIp) {
  ...
  if (isApi) {
    ...
    res.end(JSON.stringify({ ok: false, error: 'LAN 访问已关闭。在本机打开设置开启。' }))
    return true
  }

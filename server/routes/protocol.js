...
export async function handleSetMode(req, res, ctx) {
  ...
  if (!r.ok) {
    ...
    const httpCode = r.code === 'unsupported' ? 501
                   : r.code === 'no_client' ? 503
                   : r.code && /not.found|invalid/i.test(r.code) ? 404
                   : r.code && /conflict|policy/i.test(r.code) ? 409
                   : 502
    return respond(res, httpCode, { ok: false, error: r.error, code: r.code, fallback: 'send_plan_as_prompt' })
  }
...
export async function handleCancel(req, res, ctx) {
  ...
  if (!r.ok) {
    return respond(res, 200, { ok: true, cancelled: false, warning: r.error, code: r.code, fallback: 'hard_kill' })
  }
  if (ctx && ctx.cid) pushStateFor(ctx.cid)
...
export async function handleLoadSession(req, res, ctx) {
  ...
  if (!r.ok) {
    const httpCode = r.code === 'no_client' ? 503
                   : r.code && /not.found|invalid/i.test(r.code) ? 404
                   : 500
    // mcode acp "Resource not found" 返 404 的子情况, code 是 -32004 / 'resource_not_found'
    //   给前端更可读的 code
    const outCode = r.code && /not.found|resource/i.test(r.code) ? 'session_not_found' : r.code
...
export async function handleActivateSession(req, res, ctx) {
  ...
  if (ctx && ctx.cid) pushStateFor(ctx.cid)
  return respond(res, 200, { ok: true, activeSessionId: sessionId, data: r.data })
}
...
export async function handleCapabilities(_req, res) {
  ...
  return respond(res, 200, {
    ...
    notes: {
      set_mode: 'send "/plan <text>" or "/goal <text>" as a regular session/prompt — mcode parses the slash command',
      set_config_option: 'permission mode is set at mcode startup via --permission flag, not mid-session',
      cancel: 'no graceful cancel in mcode 0.1.5; use child.kill() as fallback',
      activate: 'single-session per acp client; load() a different session to switch',

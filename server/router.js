...
const ROUTES = [
  // Static + HTML
  { method: 'GET', match: (p) => p === '/' || p === '/index.html', handler: (_req, res) => {
    if (serveIndex(res) === false) {
      res.writeHead(404); res.end('not found')
    }
    return true
  }},
  { method: 'GET', match: (p) => !!p && p !== '/' && p.includes('.'), handler: (_req, res, _ctx, pathname) => {
  ...
  { method: 'OPTIONS', match: () => true, handler: (_req, res) => {
    res.writeHead(204); res.end()
    return true
  }},

  // Health
  { method: 'GET', match: (p) => p === '/api/health', handler: healthRoute.handleHealth },

  // State + SSE
  { method: 'GET', match: (p) => p === '/api/events', handler: stateRoute.handleEvents },
  { method: 'GET', match: (p) => p === '/api/state', handler: stateRoute.handleState },

  // ACP session endpoints
  { method: 'GET', match: (p) => p === '/api/acp-sessions', handler: sessionsRoute.handleAcpSessions },
  { method: 'GET', match: (p) => p === '/api/acp-session-title', handler: sessionsRoute.handleAcpSessionTitle },
  ...
  // Chat
  { method: 'POST', match: (p) => p === '/api/send', handler: chatRoute.handleSend },
  { method: 'POST', match: (p) => p === '/api/stop', handler: chatRoute.handleStop },
  { method: 'POST', match: (p) => p === '/api/cmd', handler: chatRoute.handleCmd },

  // Usage
  { method: 'POST', match: (p) => p === '/api/usage' || p === '/api/usage-trigger', handler: usageRoute.handleUsage },
  { method: 'GET',  match: (p) => p === '/api/usage-real', handler: usageRoute.handleUsageReal },
  { method: 'POST', match: (p) => p === '/api/refresh', handler: usageRoute.handleRefresh },

  // Workspace
  { method: 'POST', match: (p) => p === '/api/workspace', handler: workspaceRoute.handleWorkspace },
  { method: 'GET',  match: (p) => p === '/api/workspace/browse', handler: workspaceRoute.handleWorkspaceBrowse },

  // Settings
  { method: 'GET',  match: (p) => p === '/api/settings', handler: settingsRoute.handleGetSettings },
  { method: 'POST', match: (p) => p === '/api/settings', handler: settingsRoute.handlePostSettings },

  // Upload
  { method: 'POST', match: (p) => p === '/api/upload', handler: uploadRoute.handleUpload },

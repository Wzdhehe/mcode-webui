...
// Unit tests for server/routes/chat.js â€?handleSend
//
// Why this test exists: v0.5.bx-32 â€?handleSend must write
...
describe('handleSend â€?v0.5.bx-32 lastUsedWorkspace contract', () => {
  test('writing a real message sets cs.lastUsedWorkspace to current workspace.dir', async () => {
    ...
    assert.equal(cs.lastUsedWorkspace, '/ws-X',
  ...
  test('ask_user answer does NOT write lastUsedWorkspace', async () => {
    ...
    const ctx = { cs, cid }
    const res = fakeRes()
    await handleSend(fakeReq({ content: 'my answer', isAskAnswer: true }), res, ctx)
